...
server.tool(
  ...
  {
    agent_id: z.string().describe("Unique agent ID (e.g. 'southgate-president')"),
    name: z.string().describe("Display name (e.g. 'Southgate Research President')"),
    role: z.enum(["president", "worker", "supervisor", "bot"]).describe("Agent role"),
  },
  async ({ agent_id, name, role }) => {
    const stmt = db.prepare(
      "INSERT INTO agents (id, name, role, last_seen) VALUES (?, ?, ?, datetime('now')) ON CONFLICT(id) DO UPDATE SET name=?, role=?, last_seen=datetime('now')"
    );
...
server.tool(
  ...
  async ({ from, to, content }) => {
    ...
    db.prepare("UPDATE agents SET last_seen = datetime('now') WHERE id = ?").run(from);
...
server.tool(
  "broadcast",
  "Send a message to a channel (all agents subscribed can read it)",
  {
    from: z.string().describe("Sender agent ID"),
...
server.tool(
  ...
  "Read unread direct messages for an agent",
  {
    ...
    mark_read: z.boolean().optional().default(true).describe("Mark messages as read after fetching"),
  },
  async ({ agent_id, mark_read }) => {
    ...
    ).all(agent_id) as Array<{ id: number; from_agent: string; content: string; created_at: string }>;
...
server.tool(
  ...
  {
    channel: z.string().describe("Channel name (e.g. '#presidents')"),
    limit: z.number().optional().default(20).describe("Number of recent messages to fetch"),
  },
  async ({ channel, limit }) => {
    const messages = db.prepare(
      "SELECT id, from_agent, content, created_at FROM messages WHERE channel = ? ORDER BY created_at DESC LIMIT ?"
    ).all(channel, limit) as Array<{ id: number; from_agent: string; content: string; created_at: string }>;
...
server.tool(
  "list_agents",
  "List all registered agents on the message bus",
  {},
  async () => {
    const agents = db.prepare(
      "SELECT id, name, role, last_seen FROM agents ORDER BY last_seen DESC"
    ).all() as Array<{ id: string; name: string; role: string; last_seen: string }>;
...
server.tool(
  ...
  async () => {
    const channels = db.prepare(
      "SELECT name, description FROM channels ORDER BY name"
...
server.tool(
  ...
  {
    name: z.string().describe("Channel name (must start with #)"),
