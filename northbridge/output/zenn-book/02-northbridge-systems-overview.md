...
この章では、その運用OSを実際に担う `Northbridge Systems` が、このプロジェクトの中で何を持ち、何を持たないのかを整理する。
...
Northbridge も Southgate も、スポンサーを迂回しない。両社は提案し、準備し、運用するが、金と外部公開と不可逆操作の鍵はスポンサーが持つ。
...
つまり Northbridge は、「前へ進める会社」である前に「雑に進めない会社」だ。
- 失敗を止める
- approval gate を守る
- 仕組みを商品に変える
