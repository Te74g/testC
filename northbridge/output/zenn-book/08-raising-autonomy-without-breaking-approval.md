...
しかし Northbridge Systems は、ここをかなり厳しく見る。

理由は単純で、**自律性と無断実行は別物** だからだ。
...
これは保守的すぎるように見えるかもしれない。だが実際には、これがあるから継続的に拡張できる。
...
Northbridge はそこをかなり意識していて、まずは小さく bounded に進め、危険側は必ず gate に押し戻す。
...
AIシステムは、待つのが苦手だ。入力が無いと「前進した方がよい」と考えがちだからだ。

しかし、承認制ではそれをやってはいけない。

Northbridge 側の運用本である以上、ここを曖昧にして「だいたい自動化できる」と書くのは逃げだ。
